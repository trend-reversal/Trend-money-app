      ...
      <div className="mx-auto min-h-screen w-full max-w-[390px] bg-white">
        ...
        <section className="space-y-[11px] bg-white px-[10px] pb-7">
          {TRANSACTIONS.map((item, index) => (
            <div
              ...
              onClick={() => {
                if (item.status === "SUCCESS") {
                  router.push("/gold/transaction-success?status=success");
                }

                if (item.status === "FAILED") {
                  router.push("/gold/transaction-success?status=failed");
                }

                if (item.status === "PENDING") {
                  router.push("/gold/transaction-success?status=pending");
                }
              }}
            ...
            >
              <div
                className="flex h-[58px] w-[58px] flex-shrink-0 items-center justify-center rounded-[10px] text-center leading-tight"
                style={{
                  backgroundColor: item.bg,
                  color: item.text,
                  fontSize: item.type === "Jewellery" ? "10px" : "11px",
                  fontWeight: 700,
                  padding: item.type === "Jewellery" ? "4px" : undefined,
                }}
              >
                {item.type}
              </div>
